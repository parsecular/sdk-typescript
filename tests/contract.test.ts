...
import type { OrderbookSnapshot, Activity } from 'parsec-api/streaming';
...
if (!RUN_LIVE) {
  ...
  describe('REST: markets', () => {
    ...
    test(
      'GET /api/v1/markets supports multi-exchange filter',
      async () => {
  ...
  describe('REST: orderbook', () => {
    test(
      'GET /api/v1/orderbook returns correctly structured bids/asks',
      async () => {
  ...
  describe('WebSocket contract tests (live server)', () => {
    ...
    test(
      'connects, authenticates, subscribes, and receives valid orderbook snapshot',
      async () => {
    ...
    test(
      'batch subscribe to 2 markets receives 2 distinct snapshots',
      async () => {
    ...
    test(
      'unsubscribe stops receiving updates for that market',
      async () => {
        const [market] = await findActiveMarketsWithDepth(1);
        const ws = client.ws();
    ...
    test(
      'auth_error on invalid API key — rejects connect and does not hang',
      async () => {

...
import { APIPromise } from 'parsec-api/core/api-promise';

import util from 'node:util';
import ParsecAPI from 'parsec-api';
import { APIUserAbortError } from 'parsec-api';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['PARSEC_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: ParsecAPI) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['PARSEC_API_LOG'] = 'debug';
      const client = new ParsecAPI({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['PARSEC_API_LOG'] = 'not a log level';
      const client = new ParsecAPI({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'PARSEC_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new ParsecAPI({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['PARSEC_API_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new ParsecAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new ParsecAPI({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['PARSEC_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new ParsecAPI({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new ParsecAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new ParsecAPI({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new ParsecAPI({ apiKey: 'My API Key', fetch: testFetch });

# Parsec API TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/parsec-api.svg?label=npm%20(stable)>)](https://npmjs.org/package/parsec-api) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/parsec-api)
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install parsec-api`
...
1. Via the `PARSEC_API_LOG` environment variable
...
const client = new ParsecAPI({
  logger: logger.child({ name: 'ParsecAPI' }),
...
const client = new ParsecAPI({ fetch });
```
...
```ts
import ParsecAPI from 'npm:parsec-api';
